...
def get_valid_expiry_dates_2025():
    ...
    for thursday in all_thursdays:
        if thursday in holidays:
            valid_expiries.append(thursday - timedelta(days=1))
        else:
...
def plot_candlestick(df, title, include_oi=False):
    ...
    fig.add_trace(go.Bar(x=df['datetime'], y=df['volume'], name='Volume', yaxis='y2', opacity=0.3))
    if include_oi:
        fig.add_trace(go.Scatter(x=df['datetime'], y=df['open_interest'], name='Open Interest',
                                 yaxis='y3', line=dict(color='yellow')))
    fig.update_layout(template='plotly_dark', title=title, height=800, yaxis_title='Price', xaxis_title='Time',
                      yaxis2=dict(title="Volume", overlaying="y", side="right", showgrid=False))
    if include_oi:
        fig.update_layout(yaxis3=dict(title="Open Interest", overlaying="y", side="right",
...
def main():
    ...
    with tab2:
        ...
        expiry = st.selectbox("Select Expiry Date", valid_expiries,
                              index=valid_expiries.index(min(filter(lambda d: d >= datetime.today().date(), valid_expiries))))
